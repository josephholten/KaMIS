...
def search_for_graphs(keyword_list, graph_folder="instances"):
    ...
    matched_graphs = [{'kw': kw, 'path': [path for path in os.listdir(graph_folder) if kw in path and path[-6:] == ".graph"]} for kw in keyword_list]
    for graph in matched_graphs:
        # check if kw's are unique
        assert len(graph['path']) == 1, "kw's selecting graph are ambiguous"
        graph['path'], = graph['path']   # assign graph['path'] to the first obj in graph['path']
        graph['path'] = graph_folder + ("/" if graph_folder[-1] != "/" else "") + graph['path']
...
def get_graphs_and_labels(graph_objs):  # graph_objs of type: list of dicts, each dict needs entry 'kw' and 'path'
    for graph_obj in graph_objs:
        graph_path = graph_obj['path']
...
print("testing on graphs", *(graph['kw'] for graph in test_graphs))
track_removals = {graph['kw']: [] for graph in test_graphs} 
...
for stage in range(1, num_stages+1):
    ...
    for graph in test_graphs:
        ...
        track_removals[graph['kw']].append(len(removal))

    dtest = get_dmatrix_from_graphs(test_graphs)

for graph in test_graphs:
    print("in graph", graph['kw'], "removed a total of", sum(track_removals[graph['kw']]), "nodes", "(", *track_removals[graph['kw']], ")")

print("solution quality:")
for graph in test_graphs:
    print("number of edges:", len(graph['graph'].edges))
    # print("edges:\n", graph['graph'].edges)